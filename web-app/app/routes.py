...
def home():
    try:
        ...
        lost_items = list(
            items_collection.find({"status": "lost"}).sort("dateLost", -1).limit(5)
        )
        found_items = list(
            items_collection.find({"status": "found"}).sort("dateFound", -1).limit(5)
        )
        ...
        recent_items.sort(
            key=lambda x: x.get("dateLost", x.get("dateFound")), reverse=True
        )
